...
func (node *Node) FindSuccessor(id big.Int, reply *string) error {
	...
	logrus.Info(node.Addr, " Safe innet lock")
	if !node.innet {
	...
	*prime = node.Addr
	...
	node.RemoteCall(*prime, "Node.FindSuccessor", HashString(*prime), reply)
	return nil
}
